...
#define NUMBEROFPALLINES 16 //smallest power of 2 greater or equal to BunnyPaletteLineNames::LAST-1; has to be manually calculated in order to be built into the shaders
...
namespace BunnyPaletteLineNames {
	enum {
		...
		TBGFadeIntensity,
		Brightness, BrightnessToFreezeColors,
								  //pallineMENUPLAYERSPRITEMAPPING, pallineHEATEFFECTOFFSETS, pallinePLAYERS, pallineGEMS = pallinePLAYERS + 32,
		LAST
	};
}
namespace BunnyShaders {
	enum {
		...
		Gem, Palshift, Shadow, Brightness, SimpleFrozen,
...
public:
	SpriteModeGem(sf::Uint8 p) : SpriteMode(Shaders[BunnyShaders::Gem], p) {
		ParamAsFloat = (BunnyPaletteLineNames::Gem + Param) / float(NUMBEROFPALLINES);
	}
};

extern SpriteMode SpriteModeFrozen, SpriteModeJustHit;